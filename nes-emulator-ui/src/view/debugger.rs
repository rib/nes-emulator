...
use crate::ui::{ViewRequestSender, ViewRequest};
                    ...
                    ui.vertical_centered_justified(|ui| {
                        if self.paused {
                            if ui.button("Step In").clicked() {
                                self.view_request_sender.send(ViewRequest::InstructionStepIn);
                            }
                            if ui.button("Step Over").clicked() {
                                self.view_request_sender.send(ViewRequest::InstructionStepOver);
                            }
                            if ui.button("Step Out").clicked() {
                                self.view_request_sender.send(ViewRequest::InstructionStepOut);
                            }
                        }
                    });
                                ...
                                .horizontal(|mut strip| {
                                    ...
                                    strip.cell(|ui| {
                                        egui::ScrollArea::vertical().show(ui, |ui| {
                                            ...
                                            let (instruction, operand) = nes.peek_instruction(pc);

                                            ui.label(format!("{}", instruction.disassemble(operand.raw_operand, operand.operand)));
                                        });
                                    });
                                        ...
                                        ui.push_id("debugger_watchpoints", |ui| {
                                            ui.group(|ui| {
                                                egui::ScrollArea::vertical().show(ui, |ui| {
                                                    ...
                                                    for wp in nes.system_mut().debug.watch_points.iter() {
                                                        let addr = wp.address;
